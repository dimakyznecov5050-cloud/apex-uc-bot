...
BRAWL_STARS_TEXT = (
    ...
    'Также можно купить любой Донат который есть в игре писать - @Kurator111\n\n'
    'Мы отправим вам товар подарком.\n\n'
    '‼️ <b>Важно чтобы я был у вас в друзьях 24 часа!</b>'
)
...
def all_games_keyboard():
    ...
    markup.add(
        build_inline_button('Brawl Stars', 'game_brawl_stars', style='danger'),
...
def brawl_stars_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=1)
    for idx, (label, price, emoji) in enumerate(BRAWL_STARS_ITEMS):
        markup.add(types.InlineKeyboardButton(f'{emoji} {label} — {fmt_price(price)} ₽', callback_data=f'brawl_item_{idx}'))
    markup.add(build_inline_button('Назад', 'back_main', BACK_EMOJI_ID))
    return markup


def brawl_stars_buy_keyboard(item_idx):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        build_inline_button('Купить', f'brawl_buy_{item_idx}', BUY_EMOJI_ID, style='success'),
...
def build_payment_text(order_number, product_name, quantity_text, price, target_value, target_type):
    ...
    elif target_type == 'brawl_gift':
        lines.append(f'• Формат: {escape_html(target_value)}')
...
def notify_admin_about_paid_order(order_number):
    ...
    target_label = {'player_id': 'PUBG ID', 'telegram_username': 'Telegram username', 'brawl_gift': 'Формат выдачи'}.get(order['target_type'], 'Данные')
    admin_text = [
...
def handle_callback(call):
    ...
    if data == 'game_brawl_stars':
        edit_menu(user_id, BRAWL_STARS_TEXT, brawl_stars_keyboard())
        return

    if data.startswith('brawl_item_'):
        idx = int(data.split('_')[-1])
        label, price, emoji = BRAWL_STARS_ITEMS[idx]
        text = (
            f'<b>{emoji} Brawl Stars</b>\n\n'
            f'Товар: <b>{escape_html(label)}</b>\n'
            f'Цена: <b>{fmt_price(price)} ₽</b>\n\n'
            '🎁 Отправка подарком в игре.\n'
            '🤝 Перед покупкой убедитесь, что @Kurator111 есть у вас в друзьях уже 24 часа.\n\n'
            'Нажмите «Купить», чтобы перейти к оплате.'
        )
        edit_menu(user_id, text, brawl_stars_buy_keyboard(idx))
        return

    if data.startswith('brawl_buy_'):
        idx = int(data.split('_')[-1])
        label, price, _ = BRAWL_STARS_ITEMS[idx]
        order_number = create_order(
            user=call.from_user,
            product_type='brawl_stars',
            product_name='Brawl Stars',
            quantity_text=label,
            price=price,
            target_value='Подарком в игре',
            target_type='brawl_gift'
        )
        clear_state(user_id, keep_menu=True)
        edit_menu(
            user_id,
            build_payment_text(order_number, 'Brawl Stars', label, price, 'Подарком в игре', 'brawl_gift'),
