...
def build_inline_button(text: str, callback_data: str) -> InlineKeyboardButton:
    return InlineKeyboardButton(text=text, callback_data=callback_data)


def build_url_button(text: str, url: str) -> InlineKeyboardButton:
...
def menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [build_inline_button("🛒 Купить UC", "menu_uc")],
            [build_inline_button("🎮 Приложение [Все игры]", "menu_all_games_stub")],
            [
                build_inline_button("🔥 Популярность", "menu_popularity"),
                build_inline_button("💎 Подписки", "menu_subs"),
            ],
            [
                build_inline_button("🎟 Промокоды", "menu_promo"),
                build_inline_button("ℹ️ Информация", "menu_profile"),
            ],
            [
                build_inline_button("⭐ Telegram Stars", "menu_tgstars"),
                build_inline_button("✨ Telegram Premium", "menu_tgpremium"),
            ],
            [
                build_inline_button("💬 Поддержка", "menu_support"),
                build_inline_button("📝 Отзывы", "menu_reviews"),
            ],
        ]
    )
...
def all_games_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                build_inline_button("FC Mobile", "game_stub_fc_mobile"),
                build_inline_button("Roblox", "game_stub_roblox"),
            ],
            [
                build_inline_button("Brawl Stars", "brawl_stars_menu"),
                build_inline_button("Clash Royale", "clash_royale_menu"),
            ],
            [
                build_inline_button("Free Fire", "game_stub_free_fire"),
                build_inline_button("Mobile Legends", "game_stub_mobile_legends"),
            ],
            [build_inline_button("Mobile Legends (Russia)", "game_stub_mobile_legends_russia")],
            [build_inline_button("⬅️ Назад", "back_main")],
        ]
    )


def back_to_menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "back_main")]])
...
def payment_markup(order_number: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                build_inline_button("✅ Я оплатил", f"paid_{order_number}"),
                build_inline_button("❌ Отмена", f"user_cancel_{order_number}"),
            ]
        ]
    )
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                build_inline_button("📊 Статистика", "admin_stats"),
                build_inline_button("🎟 Промокоды", "admin_promos"),
            ],
            [build_inline_button("📢 Рассылка", "admin_mailing")],
        ]
    )
...
def render_main_menu_text(first_name: str = "Игрок") -> str:
    return (
        ...
        "• Низкие цены\n\n"
        "👇 Нажми <b>КУПИТЬ UC</b> чтобы начать"
    )
...
async def edit_menu(bot: Bot, user_id: int, text: str, reply_markup: InlineKeyboardMarkup | None = None) -> bool:
    state = get_menu_session(user_id)
...
def build_payment_text(order_number: int, product_name: str, quantity_text: str | None, price: float | int, target_value: str, target_type: str) -> str:
    ...
    for card in CARDS:
        lines.extend(
            [
                f"🏦 {card['bank']}",
                f"💳 Карта: <code>{card['card']}</code>",
                f"👤 Получатель: {escape_html(card['recipient'])}",
                "",
            ]
        )
    lines.extend(
        [
            f"💰 <b>Сумма: {fmt_price(price)} ₽</b>",
            "",
            "⚠️ <b>Важно:</b>",
...
async def notify_admin_about_paid_order(bot: Bot, order_number: int) -> None:
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                build_inline_button("✅ Выполнил", f"admin_done_{order_number}"),
                build_inline_button("❌ Отказать", f"admin_deny_{order_number}"),
            ]
        ]
    )
...
async def admin_stats_handler(callback: CallbackQuery) -> None:
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "admin_back")]])
...
async def admin_promos_handler(callback: CallbackQuery) -> None:
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [build_inline_button("⬅️ Назад", "admin_back")],
        ]
    )
...
async def promo_list_handler(callback: CallbackQuery) -> None:
    ...
    if not promos:
        ...
        text = "\n".join(lines)
    markup = InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "admin_promos")]])
    await callback.message.edit_text(text, reply_markup=markup)
    await callback.answer()
...
async def promo_delete_menu(callback: CallbackQuery) -> None:
    ...
    if not promos:
        rows.append([build_inline_button("⬅️ Назад", "admin_promos")])
...
async def process_mailing_content(message: Message, state: FSMContext) -> None:
    ...
    preview_markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                build_inline_button("✅ Подтвердить", "mailing_confirm"),
                build_inline_button("❌ Отмена", "mailing_cancel"),
            ]
        ]
    )
...
async def brawl_stars_menu_handler(callback: CallbackQuery, bot: Bot) -> None:
    rows = [[build_inline_button(f"{label} — {fmt_price(price)} ₽", f"bsel_{idx}")] for idx, (label, price) in enumerate(BRAWL_STARS_ITEMS)]
    rows.append([build_inline_button("⬅️ Назад", "menu_all_games_stub")])
    await edit_menu(
        bot,
...
async def clash_royale_menu_handler(callback: CallbackQuery, bot: Bot) -> None:
    rows = [[build_inline_button(f"{label} — {fmt_price(price)} ₽", f"crsel_{idx}")] for idx, (label, price) in enumerate(CLASH_ROYALE_ITEMS)]
    rows.append([build_inline_button("⬅️ Назад", "menu_all_games_stub")])
    await edit_menu(
        bot,
        callback.from_user.id,
...
async def menu_uc_handler(callback: CallbackQuery, bot: Bot) -> None:
    rows = [[build_inline_button(f"{uc} UC — {fmt_price(price)} ₽", f"ucsel_{uc}")] for uc, price in sorted(UC_PRICES.items())]
    rows.append([build_inline_button("⬅️ Назад", "back_main")])
    await edit_menu(bot, callback.from_user.id, "🛒 <b>ВЫБЕРИТЕ ПАКЕТ UC</b>\n\nНажмите на нужный пакет ниже.", InlineKeyboardMarkup(inline_keyboard=rows))
    await callback.answer()
...
async def uc_select_handler(callback: CallbackQuery, bot: Bot, state: FSMContext) -> None:
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("❌ Отмена", "back_main")]])
...
async def menu_popularity_handler(callback: CallbackQuery, bot: Bot) -> None:
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [build_inline_button("Популярность", "pop_menu_pop_regular")],
            [build_inline_button("Популярность для дома", "pop_menu_pop_home")],
            [build_inline_button("Популярность на последней минуте", "pop_menu_pop_last")],
            [build_inline_button("⬅️ Назад", "back_main")],
        ]
    )
...
async def popularity_submenu_handler(callback: CallbackQuery, bot: Bot) -> None:
    ...
    rows = [[build_inline_button(f"{label} — {fmt_price(price)} ₽", f"popsel_{key}_{idx}")] for idx, (label, price) in enumerate(config["prices"])]
    rows.append([build_inline_button("⬅️ Назад", "menu_popularity")])
...
async def menu_subs_handler(callback: CallbackQuery, bot: Bot) -> None:
    rows = [[build_inline_button(f"{label} — {fmt_price(price)} ₽", f"subsel_{idx}")] for idx, (label, price) in enumerate(SUBSCRIPTION_ITEMS)]
    rows.append([build_inline_button("⬅️ Назад", "back_main")])
...
async def menu_tgstars_handler(callback: CallbackQuery, bot: Bot, state: FSMContext) -> None:
    text = (
        ...
        "✏️ <b>Введите Telegram username получателя</b> (с @ или без):"
    )
    markup = InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "back_main")]])
    await edit_menu(bot, callback.from_user.id, text, markup)
...
async def menu_profile_handler(callback: CallbackQuery, bot: Bot) -> None:
    ...
    text = (
        ...
        f"✅ Куплено: {profile['completed_orders']}\n"
        f"⏳ В обработке: {profile['pending_orders']}"
    )
...
async def menu_reviews_handler(callback: CallbackQuery, bot: Bot) -> None:
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [build_url_button("Перейти к отзывам", f"https://t.me/{REVIEWS_CHANNEL}")],
            [build_inline_button("⬅️ Назад", "back_main")],
        ]
    )
    await edit_menu(bot, callback.from_user.id, "📝 <b>Отзывы</b>\n\nНажмите кнопку ниже, чтобы открыть канал с отзывами.", markup)
    await callback.answer()
...
async def menu_support_handler(callback: CallbackQuery, bot: Bot) -> None:
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [build_url_button("Написать в поддержку", f"https://t.me/{SUPPORT_USERNAME}")],
            [build_inline_button("⬅️ Назад", "back_main")],
        ]
    )
    await edit_menu(bot, callback.from_user.id, "💬 <b>Поддержка</b>\n\nРаботаем 24/7. Нажмите кнопку ниже.", markup)
    await callback.answer()
...
async def menu_promo_handler(callback: CallbackQuery, bot: Bot, state: FSMContext) -> None:
    markup = InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("❌ Отмена", "back_main")]])
    await edit_menu(bot, callback.from_user.id, "🎟 <b>Введите промокод:</b>", markup)
...
async def admin_done_handler(callback: CallbackQuery, bot: Bot) -> None:
    ...
    review_markup = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="⭐️ Оставить отзыв", url=f"https://t.me/{REVIEWS_CHANNEL}")]]
    )
    ...
    await callback.message.edit_text(f"✅ <b>ЗАКАЗ №{order_number} ВЫПОЛНЕН</b>")
    await callback.answer()
...
async def admin_deny_handler(callback: CallbackQuery, bot: Bot) -> None:
    ...
    if order_user_id:
        markup = InlineKeyboardMarkup(
            inline_keyboard=[[build_url_button("Написать в поддержку", f"https://t.me/{SUPPORT_USERNAME}")]]
        )
        await bot.send_message(order_user_id, f"❌ <b>Ваш заказ №{order_number} был отменён.</b>", reply_markup=markup)
    await callback.message.edit_text(f"❌ <b>ЗАКАЗ №{order_number} ОТМЕНЁН</b>")
    await callback.answer()
...
async def handle_uc_player_id(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    if not player_id.isdigit() or not player_id.startswith("5"):
        await edit_menu(
            ...
            InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("❌ Отмена", "back_main")]]),
        )
        return
...
async def handle_tgstars_username(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    if not username:
        await edit_menu(
            ...
            InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "back_main")]]),
        )
        return
    ...
    await edit_menu(bot, message.from_user.id, text, InlineKeyboardMarkup(inline_keyboard=[[build_inline_button("⬅️ Назад", "back_main")]]))
...
async def handle_tgpremium_username(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [build_inline_button("3 месяца — 1150₽", "tgpremium_period_0")],
            [build_inline_button("6 месяцев — 1490₽", "tgpremium_period_1")],
            [build_inline_button("12 месяцев — 2550₽", "tgpremium_period_2")],
            [build_inline_button("⬅️ Назад", "back_main")],
        ]
    )
