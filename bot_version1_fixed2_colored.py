...
def payment_markup(order_number):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        build_inline_button('Я оплатил', f'paid_{order_number}', '5253558615842785529', style='success'),
        build_inline_button('Отмена', f'user_cancel_{order_number}', '5255732655273570754', style='danger')
    )
    return markup
...
def notify_admin_about_paid_order(order_number):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        build_inline_button('Выполнить', f'admin_done_{order_number}', '5253558615842785529', style='success'),
        build_inline_button('Отказать', f'admin_deny_{order_number}', '5255732655273570754', style='danger')
    )
...
def process_mailing_content(message):
    ...
    markup.add(
        ...
        build_inline_button('Отмена', 'mailing_cancel', '5255732655273570754', style='danger')
    )
...
def handle_callback(call):
    ...
    if data == 'brawl_stars_menu':
        markup = types.InlineKeyboardMarkup(row_width=1)
        for idx, (label, price) in enumerate(BRAWL_STARS_ITEMS):
            markup.add(types.InlineKeyboardButton(f'{label} — {fmt_price(price)} ₽', callback_data=f'bsel_{idx}'))
        markup.add(build_inline_button('Назад', 'menu_all_games_stub', BACK_EMOJI_ID))
        edit_menu(
            user_id,
            '💎 <b>Brawl Stars</b>\n\n'
    ...
    if data == 'clash_royale_menu':
        markup = types.InlineKeyboardMarkup(row_width=1)
        for idx, (label, price) in enumerate(CLASH_ROYALE_ITEMS):
            markup.add(types.InlineKeyboardButton(f'{label} — {fmt_price(price)} ₽', callback_data=f'crsel_{idx}'))
        markup.add(build_inline_button('Назад', 'menu_all_games_stub', BACK_EMOJI_ID))
        edit_menu(
            user_id,
            '👑 <b>Clash Royale</b>\n\n'
    ...
    if data.startswith('bsel_'):
        ...
        clear_state(user_id, keep_menu=True)
        edit_menu(user_id, build_payment_text(order_number, 'Brawl Stars', label, price, 'Подарком', 'gift'), payment_markup(order_number))
        return

    if data.startswith('crsel_'):
        idx = int(data.split('_')[1])
        ...
        clear_state(user_id, keep_menu=True)
        edit_menu(user_id, build_payment_text(order_number, 'Clash Royale', label, price, 'Подарком', 'gift'), payment_markup(order_number))
        return
    ...
    if data.startswith('ucsel_'):
        ...
        markup = types.InlineKeyboardMarkup().add(build_inline_button('Отмена', 'back_main', '5255732655273570754', style='danger'))
...
def handle_uc_player_id(message, state):
    ...
    if not player_id.isdigit() or not player_id.startswith('5'):
        edit_menu(message.from_user.id,
                  ...
                  types.InlineKeyboardMarkup().add(build_inline_button('Отмена', 'back_main', '5255732655273570754', style='danger')))
        return
