...
@bot.callback_query_handler(func=lambda c: c.data == 'clash_royale_menu')
def clash_menu(call):
    ...
    items = [
        ...
        ('6500 — 4080 ₽','cr_6500'),
        ('14000 — 816 ₽','cr_14000'),
        ('Gold Pass — 979 ₽','cr_pass')
    ]
    for t,c in items:
        kb.add(types.InlineKeyboardButton(t,callback_data=c))
    kb.add(types.InlineKeyboardButton('⬅️ Назад',callback_data='back_main'))
    bot.edit_message_text('Выберите товар Clash Royale:',call.message.chat.id,call.message.message_id,reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith('cr_'))
def cr_buy(call):
    text = "💳 Реквизиты для оплаты:\n\n"
    for cdata in CARDS:
        text+=f"{cdata['bank']} — {cdata['card']} ({cdata['recipient']})\n"
    kb=types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton('✅ Я оплатил',callback_data='paid_cr'))
