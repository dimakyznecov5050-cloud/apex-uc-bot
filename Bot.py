...
def menu_keyboard():
    ...
    markup.add(
        types.InlineKeyboardButton('🛒 КУПИТЬ UC', callback_data='menu_uc'),
        types.InlineKeyboardButton('✨ ПОПУЛЯРНОСТЬ', callback_data='menu_popularity')
    )
    markup.add(
        types.InlineKeyboardButton('⭐️ ПОДПИСКИ', callback_data='menu_subs'),
        types.InlineKeyboardButton('🌟 TELEGRAM STARS', callback_data='menu_tgstars')
    )
    markup.add(
    ...
    markup.add(
        ...
        types.InlineKeyboardButton('⭐️ ОТЗЫВЫ', callback_data='menu_reviews')
    )
    markup.add(types.InlineKeyboardButton('📞 ПОДДЕРЖКА', callback_data='menu_support'))
    return markup


def back_to_menu_markup():
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('⬅️ В меню', callback_data='back_main'))
    return markup
...
def render_main_menu_text(first_name='Игрок'):
    return (
        ...
        '🔥 <b>Лучший магазин UC для PUBG Mobile</b>\n\n'
...
def send_or_update_main_menu(chat_id, user_id, first_name='Игрок'):
    ...
    if menu_message_id and menu_chat_id == chat_id:
        try:
            bot.edit_message_text(text, chat_id, menu_message_id, reply_markup=markup)
            clear_state(user_id, keep_menu=True)
            set_state(user_id, menu_chat_id=chat_id, menu_message_id=menu_message_id)
            return
        except Exception:
            pass
...
def notify_admin_about_paid_order(order_number):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('✅ Выполнил', callback_data=f'admin_done_{order_number}'),
        types.InlineKeyboardButton('❌ Отказано', callback_data=f'admin_deny_{order_number}')
    )
...
def promo_list(call):
    ...
    if not promos:
        ...
        text = '\n'.join(lines)
    markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('◀️ Назад', callback_data='admin_promos'))
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def process_mailing_content(message):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton('✅ Подтвердить', callback_data='mailing_confirm'),
...
def handle_callback(call):
    ...
    set_state(user_id, menu_chat_id=call.message.chat.id, menu_message_id=call.message.message_id)
    data = call.data
    ...
    if data.startswith('ucsel_'):
        ...
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('❌ Отмена', callback_data='back_main'))
    ...
    if data.startswith('pop_menu_'):
        ...
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='menu_popularity'))
    ...
    if data == 'menu_leaders':
        ...
        if not leaders:
            ...
            for i, row in enumerate(leaders):
                lines.append(f'{medals[i]} {escape_html(row["first_name"])} — {row["total_uc"]} UC')
            text = '\n'.join(lines)
        edit_menu(user_id, text, back_to_menu_markup())
        return
    ...
    if data == 'menu_reviews':
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton('⭐️ Перейти к отзывам', url=f'https://t.me/{REVIEWS_CHANNEL}'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='back_main'))
        edit_menu(user_id, '⭐️ <b>Отзывы</b>\n\nНажмите кнопку ниже, чтобы открыть канал с отзывами.', markup)
        return

    if data == 'menu_support':
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton('📞 Написать в поддержку', url=f'https://t.me/{SUPPORT_USERNAME}'))
        markup.add(types.InlineKeyboardButton('⬅️ Назад', callback_data='back_main'))
        edit_menu(user_id, '📞 <b>Поддержка</b>\n\nРаботаем 24/7. Нажмите кнопку ниже.', markup)
        return
...
def handle_uc_player_id(message, state):
    ...
    if not player_id.isdigit() or not player_id.startswith('5'):
        edit_menu(message.from_user.id,
                  ...
                  types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('❌ Отмена', callback_data='back_main')))
        return
...
def handle_tgstars_username(message):
    ...
    edit_menu(message.from_user.id, text, types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('❌ Отмена', callback_data='back_main')))
